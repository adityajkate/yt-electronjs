...
export default function ToastContainer() {
  ...
  return (
    <div className="fixed bottom-24 right-4 z-50 space-y-2">
      {toasts.map((t) => (
        <div key={t.id}
          className={`px-4 py-2.5 rounded-[8px] text-sm shadow-glass border backdrop-blur-sm animate-fade-in font-sans ${
            ...
            ${t.type === 'info' ? 'bg-surface text-text-primary border-border' : ''}
          `}>
          {t.message}
        </div>
      ))}
