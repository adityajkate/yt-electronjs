  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xs font-mono uppercase tracking-widest text-text-muted">Downloads</h2>
        <span className="text-xs text-text-muted font-mono">{downloads.length} tracks · {formatSize(totalSize)}</span>
      </div>
      ...
      {downloads.length === 0 && (
        <div className="text-center py-16">
          <div className="w-16 h-16 rounded-card bg-surface-hover border border-border flex items-center justify-center mx-auto mb-4">
            <IconDownload size={24} className="text-text-muted" />
          </div>
